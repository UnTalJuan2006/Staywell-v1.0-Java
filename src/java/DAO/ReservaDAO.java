...
public List<Reserva> listarPorUsuario(int idUsuario) throws SQLException {
    ...
    public int crearReservaConPago(Reserva reserva, Pago pago) throws SQLException {
        FacesContext context = FacesContext.getCurrentInstance();
        Usuario usuarioLogueado = (Usuario) context.getExternalContext()
                .getSessionMap().get("usuarioLogueado");
        ...
        String sqlReserva = "INSERT INTO reserva (checkin, checkout, fechaReserva, estado, nombreCliente, email, telefono, observaciones, idHabitacion, idUsuario) "
                + "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conexion = Conexion.conectar()) {
            ...
            boolean estadoAutoCommit = conexion.getAutoCommit();
            try {
                conexion.setAutoCommit(false);

                try (PreparedStatement ps = conexion.prepareStatement(sqlReserva, PreparedStatement.RETURN_GENERATED_KEYS)) {
                    ...
                    ps.setTimestamp(2, reserva.getCheckout() != null ? Timestamp.valueOf(reserva.getCheckout()) : null);
                    ps.setTimestamp(3, Timestamp.valueOf(fechaReserva));
                    ps.setString(4, reserva.getEstado() != null ? reserva.getEstado().name() : EnumEstadoReserva.ACTIVA.name());
                    ps.setString(5, reserva.getNombreCliente());
                    ps.setString(6, reserva.getEmail());
                    ps.setString(7, reserva.getTelefono());
                    ps.setString(8, reserva.getObservaciones());
                ...
                int idPago = pagoDAO.agregarPago(pago, conexion);
                if (idPago <= 0) {
                    conexion.rollback();
                    return -1;
                }
                ...
                conexion.setAutoCommit(estadoAutoCommit);
            }
